...
class MlpPolicy(nn.Module):
    def __init__(self, action_size, input_size=4):
        ...
        self.fc_pi = nn.Linear(self.action_size, self.action_size)
        self.fc_v = nn.Linear(self.action_size, 1)
...
class Agent(AgentConfig):
    ...
    def update_network(self):
        state = torch.from_numpy(np.array(self.memory['state'])).to(device).float()
        # get ratio
    ...
    def finish_path(self, length):
        state = torch.from_numpy(np.array(self.memory['state'][-length:])).to(device).float()
        reward = self.memory['reward'][-length:]
        next_state = torch.from_numpy(np.array(self.memory['next_state'][-length:])).to(device).float()
        terminal = self.memory['terminal'][-length:]
        td_target = torch.tensor(reward, device=device) + \
                    self.config['gamma'] * self.policy_network.v(next_state) * torch.tensor(terminal, device = device)
        delta = (td_target - self.policy_network.v(state)).to('cpu')
